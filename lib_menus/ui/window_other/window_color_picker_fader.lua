...
signalTable.OnLoad = function(caller,status,creator)
	if(caller.name ~= nil) then
		...
		caller.RGB._R.Target = prog;
		caller.RGB._G.Target = prog;
		caller.RGB._B.Target = prog;

		caller.CMY._C.Target = prog;
		caller.CMY._M.Target = prog;
		caller.CMY._Y.Target = prog;
	
		caller.HSB._H.Target = prog;
		caller.HSB._S.Target = prog;
		caller.HSB._BO.Target = prog;

		caller.BrightnessQuality._Q.Target = prog;
...
signalTable.RefreshEncoderSys = function(mainSettings,dummy,caller)
	...
	if(settings ~= nil) then
		...
		caller.RGB:SetChildren("System", sys)
		caller.CMY:SetChildren("System", sys)
		caller.HSB:SetChildren("System", sys)
		caller.BrightnessQuality:SetChildren("System", sys)
	end
end