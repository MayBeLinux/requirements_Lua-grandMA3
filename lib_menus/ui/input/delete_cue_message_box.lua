...
signalTable.LoadPrefClicked = function(caller,status)
	...
	if (overlay) then
		...
		temp.CueDst        = Preferences.CueDst;
		temp.CueDstOp      = Preferences.CueDstMode;
...
signalTable.SavePrefClicked = function(caller,status)
	...
	if (overlay) then
		...
		Preferences.CueDst        = temp.CueDst;
		Preferences.CueDstMode    = temp.CueDstOp;
